...
public class CircularSuffixArray {
    private static final int CUTOFF = 13; // 5;   // cutoff to insertion sort (any value between 0 and 12)
    ...
    /* ===========above is from SuffixArrayX with 3-way insertion quicksort ================= */
    ...
    public static void main(String[] args) {
        In in = new In(args[0]);
        String s = in.readString().replaceAll("\n", " ").trim();
...
//        String s = StdIn.readAll().replaceAll("\n", " ").trim();
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            String ith = "\"" + s.substring(index, Math.min(index + 50, s.length())).concat(s.substring(0, index))
                   + "\"";
